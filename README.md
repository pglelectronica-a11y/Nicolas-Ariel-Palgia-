...
- ~~La fecha de cierre en U1 genera un error de hidratación en el navegador~~ — **resuelto.** `PantallaPrincipal.tsx` calculaba `Intl.DateTimeFormat` directamente durante el render, y ese mismo locale se formatea distinto entre el servidor (Node) y el navegador. Se aplicó el mismo patrón ya usado en `CountdownTimer` y `hooks/use-theme.ts`: servidor y primer render del cliente muestran "Cierra pronto" (coinciden, sin error), y recién en un `useEffect` posterior al montaje se calcula y muestra la fecha real. Verificado en navegador con Playwright: sin errores de consola ni de hidratación, y la fecha real aparece correctamente tras el montaje.
